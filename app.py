...
class MADEngine:
    def _init_(self, gemini_key: str):
...
def display_mad_result(data: Optional[Dict], filename: str = "report.md"):
    ...
    with c2: st.metric("Status", "✅ Approved" if data.get('approved') else "⚠ Max Rounds")
...
if st.session_state.chat_mode:
    ...
    if st.button("⬅ Back"): st.session_state.chat_mode = False; st.rerun()
...
Provide comprehensive, well-organized insights based on the research data."""
            ...
            st.markdown('<div class="ui-card"><h3>✍ AI Copywriter</h3><p style="color: #78716c; font-size: 0.9rem; margin-top: -10px;">Rewrite and enhance your marketing copy in different styles to improve clarity, persuasiveness, or professionalism.</p></div>', unsafe_allow_html=True)
